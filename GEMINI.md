...
三个按钮分别对应了系统中三个不同层级的数据处理任务，它们的作用和使用场景有很明确的区别。

简单来说，它们的区别在于：“常规日常获取” vs “强行覆盖获取” vs “纯净本地计算”。

1. 📥 数据大一统更新 (拉取今日数据)
对应脚本：LOF011_daily_updater.py (常规模式)
执行动作：它会按顺序执行完整的“大一统”数据抓取流水线。包括：去 Woody 抓取因子、去外汇局抓取人民币中间价、去新浪抓取美股收盘价、去东财抓取 A 股净值等。
核心特征【防刷保护】：它受数据库 access_sync_status 表的严格保护。如果今天已经成功抓取过某项数据（比如早上已经抓过一次 Woody 了），再次点击这个按钮时，它会直接跳过网络请求，使用本地数据库里的缓存。
使用场景：每天下午 15:30 以后，用来执行每天的“例行公事”，把所有的收盘数据和基础数据拉取入库。

2. ⚡ Woody 因子强制更新 (强制刷新 Woody)
对应脚本：LOF011_daily_updater.py (附带 FORCE_WOODY_UPDATE=1 环境变量)
执行动作：执行完整的抓取流水线，但带了一把“尚方宝剑”。
核心特征【无视保护】：它会无视防刷保护机制。当执行到抓取 Woody API 的步骤时，它会强行删掉今天的“已同步”标记，强制程序重新去外网拉取一次最新的 Woody 因子数据（如最新的仓位、校准值等），并覆盖掉数据库里的旧记录。
使用场景：就像您今天遇到的情况——Woody 在盘中或者下午修改了数据（比如把 4-24 的校准值更新成了 4-27 的），而您的程序早上已经记录过旧数据了。此时点击这个按钮，就能强行把最新的正确数据拉下来覆盖！

3. 🧮 全市场静态计算 (重新计算估值)
对应脚本：LOF012_calculate_static_valuation.py
执行动作：启动纯数学计算引擎。
核心特征【绝对断网】：它绝对不会去外网爬取任何数据，执行速度极快。它只会把目前已经躺在 SQLite 数据库里的基础数据（前两个按钮拉回来的净值、价格、汇率、校准因子）拿出来，重新跑一遍 Woody 的魔法公式和矩阵兜底算法，算出一套最新的 static_valuation（静态估值）并写回数据库。
使用场景：如果您手动修改了数据库的某个错误数据，或者您在“配置中心”里手动微调了某只基金的底层替身权重，只需要点击这个计算按钮，系统就会在一两秒内根据您修改的参数重新算出所有的估值和溢价率，完全不需要去外网重新爬数据。

💡 实战连招总结：

平时日常更新点 1。
发现 Woody 数据更新了，需要洗掉脏数据时点 2。
手动改了配置或改了数据库里的错漏数据，想马上看到新的估值结果，点 3。
