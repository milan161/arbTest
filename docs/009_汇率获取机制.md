...
### 5. 日元在岸价获取（2026-08-02 新增，ETF 实时估值专用）

**函数**: `fetch_jpy_cny_spot_rate(self)`

**功能**: 从新浪财经获取 JPY/CNY 日元兑人民币在岸价（仅取新浪，不取官方中间价）。

**与 `fetch_jpy_cny_rate()` 的区别**：`fetch_jpy_cny_rate()` 优先取官方中间价、新浪在岸价仅兜底（用于静态估值）；`fetch_jpy_cny_spot_rate()` 只取新浪在岸价（用于 ETF 实时估值的 fx_base 历史）。

**数据源**: 新浪财经 `https://hq.sinajs.cn/list=fx_sjpycny`（HTTPS，本机直连可达）

**单位转换**: 新浪返回每1日元汇率（如 0.0428），函数内部 ×100 转为每100日元（如 4.28），与数据库 `jpy_cny_mid` 同口径。
...
```python
{
    '日期': '2026-08-01',
    '时间': '04:58:37',
    'jpy_cny_spot': 4.2849,  # 每100日元
    '来源': '新浪财经 fx_sjpycny'
}
```

**落库管道**: `daily_updater.py` step3（**9:20 清晨刷新**，由 main.py `morning_refresh_scheduler` 触发，与 USD 在岸价同管道同时点）每日把 JPY 在岸价通过 `upsert_exchange_rate(jpy_cny_spot=...)` 落库为历史。
- **确定时点**：9:20 北京时间（上午刷新，抓当天早盘在岸价），与 USD 在岸价同一管道。
- **本机直连**：新浪是国内站点，本机直连可达，**不用东京 VPS**（东京 VPS 只管 Yahoo ^N225 等境外站点）。
- **历史回退**：历史日期无 `jpy_cny_spot` 时，实时估值 `fx_base` 用 `COALESCE(jpy_cny_spot, jpy_cny_mid)` 回退中间价。

### 6. 汇率用途铁律：LOF vs ETF（2026-08-02 确立，用户反复强调）

汇率用途按**基金类型**区分，不是一刀切：

| 基金类型 | 静态估值 / 溢价率 | 实时估值（rt_val） | 说明 |
|:------:|:------------:|:------------:|:-----|
| LOF（多篮子） | 中间价 | 中间价 | LOF 两端都用中间价（`jpy_cny_mid` / `usd_cny_mid`） |
| ETF（单 ETF） | 中间价 | 在岸价 | ETF 静态用中间价、实时用在岸价（`jpy_cny_spot` / `usd_cny_spot`，base/current 同口径） |

> **QDII日本 4 只（513000/159866/513520/513880）都是 ETF** → 实时估值用在岸价。

**为什么 ETF 实时要用在岸价**：woody 实时公式两端（base + current）都用在岸价。若 base 用中间价、current 用在岸价，两端口径不一致，与 woody 实时偏差 ~0.15%。LOF 则不同——LOF 实时估值走篮子加权变化率矩阵，FX 只是转换因子，用中间价即可（woody LOF 实时也用中间价）。

**此前 AI 的错误**：先误把"静态用中间价"错扩成"所有实时也必须用中间价"（AGENTS.md 原 §7.3.11），又误把"ETF 实时用在岸价"扩成"所有基金实时都在岸价"——均已纠正（AGENTS.md §7.3.11/§7.3.14）。
