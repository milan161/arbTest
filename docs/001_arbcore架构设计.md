...
LOF 基金套利监控系统旨在为专业投资提供**一个端到端的、高可用的、极低延迟**的套利决策支持环境，通过云地协同数据采集与多源分布行情引擎，确保在瞬息万变的套利窗口期提供最精准的溢价参考。
...
- **职责**：极速实时行情网关与 API 中心
- **基座**：标准化 `BaseApp` 启动，内嵌 IB、QMT、达信长连接池
- **特征**：提供 WebSocket 实时推送，支持 A 股盘口五档行情与美股夜盘快照

### 2.4 渲染 (Renderer) - LOF03
- **职责**：高性能监控面板生成
...
+-----------------------------v-----------------------------+
|                   业务服务层 (Services)                   |
|  (Flask API / SocketIO 实时推送 / TradeManager 交易网关)      |
+-----------------------------+-----------------------------+
                              |
+-----------------------------v-----------------------------+
|                   算法与核心层 (Core)                     |
...
- **MarketManager**：汇率指数 ETF 历史价格
- **SystemManager**：同步状态健康监测数据湖备份

### 4.2 统一基座 (BaseApp)
所有核心脚本均继承 `BaseApp`，获得以下开箱即用的能力
- **NO_PROXY**：自动屏蔽系统代理，确保国内 API 请求不因梯子挂掉或被反爬
- **Logger**：统一格式的工业级日志，支持等级动态调整
- **DB Connector**：自动定位 `database/arb_master.db`（项目根目录下的 database 子目录）
...
系统通过 `unified_fund_history` 等核心表实现“动静结合”：
...
├── fetchers/        # 数据获取模块
  ...
  ├── woody_api_service.py    # 统一 Woody API 服务 (提取解析与备份)
  ├── woody_telegram_client.py# Woody Telegram 接口客户端
  ├── ib_reader.py            # IB 盈透实时行情与交易基座
...
**实现方式
这些工具函数目前直接集成在各个模块中，没有单独的 utils 目录
