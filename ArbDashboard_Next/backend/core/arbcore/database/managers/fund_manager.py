...
class FundManager(BaseManager):
    ...
    def sync_unified_fund_list(self, fund_list: List[Dict[str, str]]):
        with self.lock:
            try:
                conn = self._get_conn()
    ...
    def get_unified_fund_list(self) -> List[Dict[str, str]]:
        conn = self._get_conn()
        cursor = conn.execute("SELECT category, fund_code, fund_name, related_index FROM unified_fund_list")
