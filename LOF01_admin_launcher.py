...
ADMIN_LOGS = {
    "011": os.path.join(LOGS_DIR, "011 basic基础日志.log"),
    "012": os.path.join(LOGS_DIR, "012 LOF历史日志.log"),
    "013": os.path.join(LOGS_DIR, "admin_013.log"),
    "woody": os.path.join(LOGS_DIR, "admin_011下午.log"),
}
...
def _load_admin_status():
    ...
    return {
        ...
        "013": {"status": "unknown", "last_run": None, "message": ""},
...
@app.route("/admin/run/011", methods=["GET", "POST"])
def admin_run_011():
    _run_script_async("LOF011_generate_basic_data.py", "011", force_woody=False)
    return jsonify({"status": "started", "task": "011"}), 200


@app.route("/admin/run/012", methods=["GET", "POST"])
def admin_run_012():
    _run_script_async("LOF012_generate_lof_data.py", "012", force_woody=False)
    return jsonify({"status": "started", "task": "012"}), 200

@app.route("/admin/run/013", methods=["GET", "POST"])
def admin_run_013():
    _run_script_async("LOF013_generate_futures_data.py", "013")
    return jsonify({"status": "started", "task": "013"}), 200

@app.route("/admin/run/woody", methods=["GET", "POST"])
def admin_run_woody():
    _run_script_async("LOF011_generate_basic_data.py", "woody", force_woody=True)
    return jsonify({"status": "started", "task": "woody"}), 200

@app.route("/admin/run/calib", methods=["GET", "POST"])
def admin_run_calib():
    _run_script_async("LOF011_generate_basic_data.py", "calib", extra_args=["--calib-only"])
